...
import pt.unl.fct.di.apdc.avaliacaoindividual.util.RoleUpdateData;

@Path("/roleupdate")
@Produces(MediaType.APPLICATION_JSON + ";charset=utf-8")
public class RoleUpdateResource {

	private static final Logger LOG = Logger.getLogger(RoleUpdateResource.class.getName());
	private final Datastore datastore = DatastoreOptions.getDefaultInstance().getService();

	public RoleUpdateResource() {
	}

	@PUT
	@Path("/")
	@Consumes(MediaType.APPLICATION_JSON)
	public Response doRoleUpdate(RoleUpdateData data) {
			...
			if (tokenEnd < System.currentTimeMillis()) {
				...
				if (target == null) {
					...
					if (!data.possibleUpdate(user.getString("user_role"), target.getString("user_role"))) {
						tra.rollback();
						return Response.status(Status.FORBIDDEN).entity("Invalid promotion.").build();
					} else {
						try {
							target = Entity.newBuilder(target).set("user_role", data.newRole).build();
							tra.update(target);
							tra.commit();
